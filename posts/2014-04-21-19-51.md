...
All code samples in this post compile with `rustc` from commit `6c3bdbe` 
(compiled on `2014-04-25, 12:01-07`). 
Code samples with an `// Output:` comment are tested before uploaded.

All code samples *should* correspond to a complete program that is
...
(Note about Rust: Currently, syntax extensions require a special compiler 
directive `#[phase(syntax)]` to work. The `phase` directive must be enabled 
...
automated by the Rust compiler and provides seamless integration with regular 
dynamic regexes.
...
written with `macro_rules!` because they are simple to write.

The second way to write a macro is via a syntax extension (also known as a 
"procedural macro"). This is done by a compile time hook that lets you execute 
arbitrary Rust code, rewrite the abstract syntax to whatever you want and opens 
up access to the Rust compiler's parser. In short, you get a lot of power. And 
it's enough to implement native regexes. The obvious drawback is that they are 
...
extension functionality itself.
...
[`~MacResult`](http://static.rust-lang.org/doc/master/syntax/ext/base/trait.MacResult.html)
...
helper type to construct a value that has type `~MacResult`. Finally, we see 
that `MacExpr::new` has type `@Expr -> ~MacResult`. Ah ha! So if we can build 
an expression, then we can build a `~MacResult`, and therefore be done with our 
macro expansion.

(Note: The AST types are still using the old managed pointer syntax. At some 
point, this will be changed before the `@` sigil is completely scrubbed from 
the language.)
...
wrote two helper functions `int_literal` and `dummy_expr` to encapsulate some 
...
<code
  ...
  data-gist-hide-footer="true"
  data-gist-hide-line-numbers="true"
  data-gist-highlight-line="11,23-28,38-68"
 ></code>
...
Finally, this can be used the same way as last time, except we call 
...
### Why do you keep saying "well behaving clients"?
...
benchmarks, where there is nearly an order of magnitude improvement in all of 
them.
...
* The
  [infrastructure used to compile, check and upload](http://localhost)
