...
### Specs
  ...
  * 14" 1600x900 display
  * Intel HD Graphics 4000, [b]no discrete graphics card
  * 4GB memory
...
On 66% brightness, it seems like about 8 hours seems to be the sweet spot for 
...
included the updated drivers for Intel's HD 4000 graphics (and possibly wifi?), 
...
parameter is no longer needed. (I also tried one of the snapshot installers, 
but it was having trouble setting up the mount points.)
...
`MODULES` in rc.conf. This is my thinkfan `/etc/thinkfan.conf`:
...
I've tested it with the utilities in the [b]cpuburn[/b] package and it seems to 
