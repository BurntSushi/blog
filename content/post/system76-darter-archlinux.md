...
the only way to go above 16GB of memory is to get on with a bundled NVIDIA
...
What I should have done was copy the relevant configuration files from
`Pop!_OS` when I booted to it, since X11 worked fine there. Although, it's
likely it was using an older Linux kernel (I'm on 5.4.14) which maybe could
